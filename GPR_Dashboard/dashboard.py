    ...
    if st.button("🚀 오늘의 지정학 뉴스 업데이트", use_container_width=True):
        with st.spinner("최신 연합뉴스 국제망 초고속 스캔 중..."):
            try:
                ...
                st.rerun()  # 스캔 완료 즉시 화면을 갱신하여 뉴스를 바로 띄움
...
# ------------------------------------------------------------------------------
# LEFT COLUMN : 뉴스 레이더 (작게, 위험도 큰 순으로 가로 3개 배치)
# ------------------------------------------------------------------------------
with col1:
    ...
    if os.path.exists(news_file):
        try:
            ...
            if not news_df.empty:
                # 📌 위험도 점수(risk_score)가 높은 순서대로 강제 정렬
                ...
                # 📌 캡틴의 오더: "작게 가로로 3개 들어가게"
                news_cols = st.columns(3)
                
                for i, (idx, row) in enumerate(paged_df.iterrows()):
                    with news_cols[i % 3]:  # 가로 3칸에 순서대로 할당
                        ...
                        # 좁은 가로 공간에 맞게 텍스트 길이 최소화
                        st.markdown(f"**[{badge}]**")
                        st.markdown(f"**[{row['title']}]({row['url']})**")
                        st.caption(f"🎯 파급: {row['expected_impact']}")
                        st.caption(f"📝 {str(row['summary'])[:50]}...") # 요약본은 50자로 제한하여 작게 유지
