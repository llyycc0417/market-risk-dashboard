...
with col_btn:
    if st.button("🚀 오늘의 지정학 뉴스 업데이트", use_container_width=True):
        with st.spinner("최신 연합뉴스 국제망 스캔 중..."):
            try:
                news_updater.update_news()  # 다이렉트 함수 호출로 개조
                st.session_state.news_page = 0  # 업데이트 시 첫 페이지로 초기화
...
with col1:
    ...
    if os.path.exists(news_file):
        ...
        st.info("📢 아직 오늘의 뉴스 스캔 기록이 없습니다. 상단 [오늘의 지정학 뉴스 업데이트]를 눌러 레이더를 가동하십시오!")
...
if st.button("📈 금융 시장 위험도 실시간 진단 시작", use_container_width=True):
    with st.spinner("거시경제 지표 및 FINRA 마진부채 데이터를 분석 중..."):
        try:
            ...
            with m_col3:
                st.metric(label="降低 하이일드 스프레드", value=f"{metrics['hy_spread']['value']:.2f}%", delta=f"{metrics['hy_spread']['change']:.2f}%")
            with m_col4:
